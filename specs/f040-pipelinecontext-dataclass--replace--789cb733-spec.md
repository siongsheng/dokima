...
Position: The existing spec already contains Impact and What Changed sections, but the headers don't match expected format. Tasks use Task N: instead of ### Task N: with bold field labels. I'll correct the formatting while preserving ALL content. (High confidence)
    
    
    
    F040: PipelineContext dataclass — replace 20+ module-level globals
    ...
    Based on analysis of affected source files:
