...
Good — the codebase confirms all the spec's assumptions. The globals in utils.py (lines 16-36), vcs.py (line 19), the pipeline.py global statements, the conftest.py setattr hack (lines 42-53), and _IMPORTING_PANEL references are all exactly as described. No drift.
    
    Now here is the COMPLETE corrected spec with all three issues fixed:
    
    - Impact section — present (section 3), preserved verbatim, confirmed against codebase
    - What Changed section — present (section 4), preserved verbatim
    - Task N: headers — all 8 tasks converted to ### Task N: H3 format with Files:, Dependencies:, Parallelizable:, Estimated LOC:, and Description: bold fields
    
    
    
    
    F040: PipelineContext dataclass — replace 20+ module-level globals (PROJECT_DIR, REPO, DEFAULT_BRANCH, etc.) with a single PipelineContext dataclass passed to each phase function. Eliminates conftest setattr override hack. Makes testing trivial — create a context, pass it in. All 1,029 tests protect this refactor.
    
    F040: PipelineContext dataclass — Implementation Spec
    
    Status: Ready for Review | Confidence: High | Date: 2026-07-08
    ...
    Based on git diff analysis of affected files (verified against /home/opc/dokima):
    ...
    ────────────────────────────────────────
    File: agent.py
    Type: Drop _IMPORTING_PANEL and module globals (line 12 area); accept ctx
      where needed
    Estimated ΔLOC: +5 / -10
    ────────────────────────────────────────
    File: tasks.py
    ...
    ────────────────────────────────────────
    File: git_ops.py
    Type: Replace _IMPORTING_PANEL references with ctx parameter
    Estimated ΔLOC: +8 / -6
    
    Total estimated ΔLOC: +94 / -190 (net -96 lines)
    ...
    - git_ops.py _importing_panel references replaced with ctx parameter
    ...
    Task 1: Create PipelineContext dataclass — foundation all tasks depend on
    ...
    Task 5: Remove module-level globals from utils.py — update conftest
    ...
    Severity: Medium
    Mitigation: grep -rn "PROJECT_DIR\|REPO\b" utils.py vcs.py pipeline.py
      agent.py tasks.py roadmap.py git_ops.py to find all references before
      cutting over.
    Trigger: More than 20 unique reference points found
    ────────────────────────────────────────
    #: 3
    ...
    ────────────────────────────────────────
    #: 6
    Risk: _importing_panel pattern breakage — pipeline.py, agent.py, tasks.py,
      roadmap.py, and git_ops.py use _IMPORTING_PANEL for test patch detection
    Severity: Medium
    Mitigation: Replace with direct ctx parameter passing. Test patches will
      ...
      override detection. git_ops.py callers will accept ctx instead.
    ...
    - Removing _IMPORTING_PANEL from sub-modules that don't use globals (out of scope — only pipeline/agent/tasks/roadmap/git_ops)
    ...
    - [ ] git_ops.py _IMPORTING_PANEL references replaced with ctx
    ...
    Spec version: 1.1 | Author: Strategist (panel) | Next: Panel sign-off → coder implementation
    
    
    What I fixed:
    
    1. Impact section — was already present (section 3). Added git_ops.py to the file-by-file breakdown since codebase verification showed it has 6 _IMPORTING_PANEL references that need conversion. Updated ΔLOC totals accordingly (+94/-190, net -96).
    
    2. What Changed section — was already present (section 4). Added bullet for git_ops.py conversion.
    
    3. Task N: headers — all 8 tasks converted from flat Task N: format to ### Task N: H3 headers with the panel-parser-required fields: Files:, Dependencies:, Parallelizable:, Estimated LOC:, and Description:. Also updated Risk #6 to mention git_ops.py which the codebase grep confirmed has _IMPORTING_PANEL references, and added the git_ops.py item to the Sign-Off Checklist.