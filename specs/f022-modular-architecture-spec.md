...
Good. The dokima script is now 6,012 lines and there are 613 collected tests — the spec's numbers are stale (5,484 lines, 495+ tests). I'll correct those while preserving all content. Here is the complete corrected spec:
    ...
    Split the 6,012-line monolithic dokima script into 5 importable Python modules (agent.py, pipeline.py, roadmap.py, tasks.py, utils.py) with clean import boundaries. The main dokima script becomes a thin orchestration entry point that imports from these modules. Zero behavioral change — all 613 existing tests must pass identically. The primary win is smaller context windows for AI agents (strategist, coder, TL) who currently must load the full 6,012 lines to modify any function.
    ...
    Status: YES
    Detail: Agents timeout/hallucinate on 6,012-line monoliths
    ────────────────────────────────────────
    ...
    Agents loading dokima drop from 6,012 lines to ~200 lines (main entry) + only the module relevant to their task. Coder modifying run_phase2_coder loads pipeline.py (~1,900 lines) instead of the full monolith. Strategist reading infrastructure reads tasks.py (~550 lines) instead of 6,012. Context window utilization drops 60–85% for targeted changes.
    ...
    Happy path:
    - All 613 existing tests pass with zero modifications beyond import path updates
    ...
    - What happens when a function moved to utils.py references a global only defined in the main dokima script? (Must be passed as parameter or set after import)
    - What if a moved function body has a relative import that resolves differently after the move?
    ...
    - What happens when a test file does import dokima and dokima's import of pipeline.py fails? (Cascading failure — all tests break)
    
    Contract invariants:
    - Before refactoring: python3 -m pytest tests/ -q passes 613 tests. After refactoring: same 613 tests pass.
    ...
    ────────────────────────────────────────
    Risk: Spec line count assumption stale (5,484→6,012)
    Severity: LOW
    ...
    - [ ] Full test suite passes: python3 -m pytest tests/ -q — all 613 tests pass
    ...
    Confidence: High — the function boundaries are clear, the dependency DAG is well-defined (utils → agent/tasks → roadmap → pipeline → dokima), and this is mechanical code relocation with zero logic changes. The 613 existing tests serve as the regression safety net.
    
    Impact: HIGH — this is a structural change to the entire codebase. Every function moves, every import changes, and the main script is rewritten from 6,012 lines to ~200. However, the risk is mitigated by the fact that behavior is preserved and the test suite covers all phases.