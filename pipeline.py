...
def run_fix_mode_issue(ctx, project_dir, issue_number):
    ...
    4. Create fix/issue-{N} branch, spawn coder via run_phase2_coder(ctx, mode="fix")
    ...
    coder_result = run_phase2_coder(ctx, 
        feature=feature,
        spec=coder_prompt,
        spec_path="",
        tasks_extract_path="",
    ...
    if not vet_result.get("coder_failed"):
        ...
        nm_result = run_phase4_nm(ctx, feature, branch, "MEDIUM", pr_url)
        ...
        if coder_clarify:
            ...
            if user_answers:
                ...
                coder_output = spawn_agent("coder", ["ai-coding-best-practices-lite"],
                                           coder_prompt_refine, cwd=ctx.project_dir, fallback_model=FALLBACK_MODELS.get("coder"))
...
def run_phase3_vet(ctx, feature, branch, pr_sections, impact, spec_path, depth="vet", confidence="High"):
    ...
    # F046: Branch guard — refuse to test on ctx.default_branch
    if branch == ctx.default_branch:
        print(f"  FATAL: vet phase must run on feature branch, not {ctx.default_branch}", flush=True)
        halt_and_revert(
            f"vet: branch guard — expected feature branch, got {ctx.default_branch}",
            "PHASE 3 (vet)", branch
        )
...
Report: what was broken, what you fixed, commit hash."""
            spawn_agent("coder", ["ai-coding-best-practices-lite"],
                        fix_prompt, cwd=ctx.project_dir, fallback_model=FALLBACK_MODELS.get("coder"))
...
RELEASE determination: Check if this feature introduces new functionality, API changes, or breaking changes visible to users. YES → recommend semver level (patch/minor/major). NO → no release needed."""
    tl_output = spawn_agent("tech-lead", ["adversarial-review-lite", "ponytail-guard"],
                            tl_prompt, cwd=ctx.project_dir, fallback_model=FALLBACK_MODELS.get("tech-lead"))
...
def run_phase1_strategist(ctx, feature, user_answers_prefill):
    ...
    _orig_reasoning, _orig_yaml, _strat_config = _configure_strategist_reasoning(ctx.profiles_dir)

    # Check for existing spec at standard path BEFORE spawning strategist.
    # If a human or prior pipeline already wrote a spec, it is the AUTHORITATIVE SOURCE —
    # the strategist must validate and refine it, not replace it.
    _refine_note = _check_existing_spec(ctx.project_dir, feature)
    ...
    strat_prompt = _build_strategist_prompt(ctx.project_dir, feature, user_answers_prefill, _refine_note, _ref_context)

    strat_output = spawn_agent("strategist", ["spec-strategist-lite", "ponytail-guard"], strat_prompt, timeout=300, cwd=ctx.project_dir, fallback_model=FALLBACK_MODELS.get("strategist"))
    ...
    strat_output = _handle_dag_reprompt(strat_output, feature, ctx.project_dir)

    _restore_strategist_config(_orig_reasoning, _orig_yaml, _strat_config)


    # ── Interview gate ──
    strat_output = _handle_interview_gate(strat_output, strat_prompt, feature, user_answers_prefill, ctx.project_dir)
    ...
    # Quality gate: validate spec quality, re-prompt once on failure
    spec, strat_output = _check_spec_quality_gate(spec, strat_output, feature, ctx.project_dir)
    ...
    spec_path, tasks_extract_path = _save_spec_and_extract_tasks(spec, feature, strat_output, ctx.project_dir)

    pr_sections = extract_pr_sections(spec, feature)
    print(f"  PR sections extracted: {len(pr_sections)} chars", flush=True)

    # Tech Lead spec pre-review
    _run_tech_lead_prereview(spec_path, ctx.project_dir)

    # Orchestra gate
    confidence, impact, depth, mode, branch = _compute_depth_and_orchestrator_gate(spec, feature, ctx.project_dir)

    # Human gate
    spec, pr_sections, tasks_extract_path = _run_human_gate(feature, depth, confidence, impact, branch, spec_path, tasks_extract_path, spec, pr_sections, ctx.project_dir)

    # ADR creation
    _create_adr(feature, spec_path, spec, ctx.project_dir, ctx.real_home)
...
def run_pipeline(ctx, feature, is_next, is_continuous, user_answers_prefill, resume=None):
    ...
    _status_update(feature=feature, project=ctx.project_dir, log_path=ctx.output_log)
    ...
    if not _phase_should_skip(phases_completed, "strategist", resume):
        strat_result = run_phase1_strategist(ctx, feature, user_answers_prefill)
    ...
    if exec_mode == "per_task_spawn":
        ...
        if depth == "vet" and not coder_failed:
            ...
            continue_loop = run_post_pipeline(ctx, feature, is_next, is_continuous, True, pr_url, verdict, impact, branch, spec_path, strat_output, mode)
            if not continue_loop:
                return 0
            return 0
    ...
    if exec_mode == "single_session" and not coder_failed:
        coder_result = run_phase2_coder(ctx, feature, spec, spec_path, tasks_extract_path, pr_sections, branch, depth, mode, is_next)
    ...
    if not coder_failed and depth in ("vet+nm", "full"):
        vet_result = run_phase3_vet(ctx, feature, branch, pr_sections, impact, spec_path, depth=depth, confidence=confidence)
    ...
    # Phase 4: nm
    if not coder_failed and depth in ("vet+nm", "full"):
        nm_result = run_phase4_nm(ctx, feature, branch, impact, pr_url)
    ...
    if not coder_failed and depth == "full":
        tl_result = run_phase5_tech_lead(ctx, feature, pr_url, branch, spec_path, impact, nm_output=nm_output)
    ...
    # Post pipeline
    continue_loop = True
    continue_loop = run_post_pipeline(ctx, feature, is_next, is_continuous, continue_loop, pr_url, verdict, impact, branch, spec_path, strat_output, mode)
