...
| `glab` CLI (GitLab) | `glab --version` (if using GitLab) |
...
| GitHub or GitLab remote configured | `git remote get-url origin` returns a GitHub or GitLab URL |
...
hermes --profile strategist config set terminal.env_passthrough '[GH_TOKEN, GITHUB_TOKEN, GITLAB_TOKEN, GLAB_TOKEN, HERMES_HOME, HOME]'
...
hermes --profile coder config set terminal.env_passthrough '[GH_TOKEN, GITHUB_TOKEN, GITLAB_TOKEN, GLAB_TOKEN, HERMES_HOME, HOME]'
...
hermes --profile tech-lead config set terminal.env_passthrough '[GH_TOKEN, GITHUB_TOKEN, GITLAB_TOKEN, GLAB_TOKEN, HERMES_HOME, HOME]'
```
...
### 2.4b Set up GitLab (if using GitLab)
...
# Add GitLab token to shared env
echo 'GITLAB_TOKEN=glpat-...' >> ~/.hermes/shared.env
echo 'GLAB_TOKEN=glpat-...' >> ~/.hermes/shared.env
```

Token needs: `api`, `read_repository`, `write_repository` scopes. Both `GITLAB_TOKEN` and `GLAB_TOKEN` are recommended — dokima checks `GITLAB_TOKEN` first, falls back to `GLAB_TOKEN`.

The panel auto-detects GitLab from your git remote URL. Override with `--vcs gitlab` or `PANEL_VCS=gitlab`.
...
### 3.3 Git remote must be GitHub or GitLab

The panel auto-detects `owner/repo` from `git remote get-url origin`. Supports HTTPS and SSH formats for both GitHub and GitLab:
...
Override with `--vcs <github|gitlab>` or `PANEL_VCS=gitlab`.
