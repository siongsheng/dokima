...
def _extract_convention_candidates(tl_output: str) -> list[str]:
    ...
    Filters out one-off blocker descriptions (e.g., specific function references).
    Only captures rules with convention keywords: no, never, always, must, every, all.
    """
    if not tl_output or not tl_output.strip():
        return []

    convention_kw = re.compile(r'\b(no|never|always|must|every|all)\b', re.IGNORECASE)
    one_off_pat = re.compile(r'\b\w+\(\)')  # function calls → one-off

    candidates = []
    seen = set()
    for line in tl_output.split('\n'):
        if 'BLOCKER' not in line.upper():
            continue

        # Filter out one-off: has function call reference
        if one_off_pat.search(line):
            continue

        # Check for convention keywords anywhere in the line
        if not convention_kw.search(line):
            continue

        # Extract clean rule text: strip BLOCKER prefix and formatting
        cleaned = line

        # Strip markdown bold/markers from both ends
        cleaned = re.sub(r'^\s*\**\d+\.\s*\**', '', cleaned)
        cleaned = re.sub(r'\**\s*$', '', cleaned)

        # Extract text after BLOCKER[:-\s]
        match = re.search(r'BLOCKER[:\- ]+\s*(.+)', cleaned, re.IGNORECASE)
        if match:
            cleaned = match.group(1).strip()

        # Strip control characters, keep printable + tab + space
        cleaned = ''.join(c for c in cleaned if c.isprintable() or c in ('\t', ' '))
        cleaned = cleaned.strip()

        # Enforce max length (200 chars — Security §9)
        cleaned = cleaned[:200]
...
def _append_convention_rules(rules: list[str], conventions_path: str) -> int:
    ...
    try:
        ...
        if file_content.strip():
            # Find Anti-Patterns section and extract existing bullet rules
            section_start = file_content.find(anti_patterns_section)
            if section_start >= 0:
                section_text = file_content[section_start:]
                # Extract existing bullet points: lines starting with "- "
                for line in section_text.split('\n'):
                    stripped = line.strip()
                    if stripped.startswith('- '):
                        existing_rules.add(stripped[2:].strip())

            # Determine where to append
            append_pos = len(file_content)
            if section_start >= 0:
                # Find end of Anti-Patterns section (next ## header)
                after_section = file_content[section_start + len(anti_patterns_section):]
                next_header = re.search(r'\n## ', after_section)
                if next_header:
                    append_pos = section_start + len(anti_patterns_section) + next_header.start()

            # Filter to genuinely new rules
            new_rules = [r for r in rules if r.strip() not in existing_rules]
            if not new_rules:
                return 0

            # Build append text
            append_lines = ['\n' + '- ' + r for r in new_rules]
            append_text = ''.join(append_lines)

            # Insert before next section if section exists, otherwise at end
            if section_start >= 0:
                new_content = (file_content[:append_pos] + append_text +
                               file_content[append_pos:])
            else:
                # No Anti-Patterns section yet — create it
                new_content = (file_content.rstrip('\n') + '\n\n' +
                               anti_patterns_section + '\n' +
                               ''.join(append_lines) + '\n')
        else:
            # File doesn't exist or is empty — create from scratch
            new_rules = [r.strip() for r in rules if r.strip()]
            if not new_rules:
                return 0
            new_content = ('# Dokima Conventions\n\n' +
                           anti_patterns_section + '\n' +
                           '\n'.join('- ' + r for r in new_rules) + '\n')
        ...
        print(f"  ⚠ F033: Failed to append convention rules: {e}", flush=True)
        return 0
